...
class NewTaskDialog extends StatefulWidget{
    const NewTaskDialog({required this.customerId, required this.addressId, required this.phones, this.building = false, super.key});
...
class _NewTaskDialogState extends State<NewTaskDialog> {
    ...
    late String buildingReason;
    List<String> buildingReasons = [];
    ...
    void _getReasons() async {
        try {
            ...
            buildingReasons = [];
            buildingReason = buildingReasons.first;
            appealTypes = [];
    ...
    void _createTask(BuildContext context) async {
        ...
        if (employee == null){
            ...
            try{
                final bool isBuilding = context.mounted? DefaultTabController.of(context).index == 1 : false;
                final int id = await createTask(isBuilding? buildingType : type, isBuilding? null : widget.customerId, employee, reason, isBuilding? widget.addressId : null, commentController.text,
                ...
                if (context.mounted){
                    Navigator.pop(context, {
                        'building': isBuilding,
                        'type': {
                            'id': isBuilding? buildingType : type,
                            'name': isBuilding? buildingType == 38?
                                'Магистраль выезд на ремонт' : buildingType == 48?
                                                    ...
                                                    children: [
                                                        ...
                                                        SizedBox(
                                                            ...
                                                            child: DropdownButtonFormField(
                                                                ...
                                                                initialValue: buildingReason,
                                                                items: buildingReasons.map((e) {
                                                                    return DropdownMenuItem(value: e, child: Text(e));
                                                                }).toList(),
                                                                onChanged: (v){
                                                                    setState(() {
                        ...
                        builder: (actionsContext) {
                            return AnimatedBuilder(
                                ...
                                builder: (context, child) {
                                    final isBuilding = DefaultTabController.of(actionsContext).index == 1;
                                    bool disabled = false;

                                    if (isBuilding && widget.addressId == null) {
                                        disabled = true;
                                    }
                                    else if (phoneController.text.isEmpty) {
                                        if (!isBuilding) {
                                            disabled = true;
                                        } else if (buildingType != 38) {
                                            disabled = true;
                                        }
                                    }
